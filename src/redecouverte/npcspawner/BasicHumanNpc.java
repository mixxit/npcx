...
public class BasicHumanNpc extends BasicNpc {
    ...
    public void think() {
        ...
        if (this.parent.pathgroup != null) {
            ...
            if (following == null && attacking == null) {
        ...
        if (following instanceof LivingEntity) {
            if (this.following != null) {
                ...
                if (this.health == 0 || this.following.getHealth() == 0) {
                    // they're dead, stop following
                    this.following = null;
                    this.attacking = null;
                } else {
                    // they're alive lets check distance
                    double x1 = this.following.getLocation().getX();
                    double y1 = this.following.getLocation().getY();
                    double z1 = this.following.getLocation().getZ();
                    double x2 = this.getBukkitEntity().getLocation().getX();
                    double y2 = this.getBukkitEntity().getLocation().getY();
                    double z2 = this.getBukkitEntity().getLocation().getZ();
    ...
    private void movecloserLocation(double x, double y, double z, float pitch, float yaw) {
        try {
            ...
            if (diffx < 1 && diffx > -1 && diffy < 1 && diffy > -1 && diffz < 1 && diffz > -1) {
                ...
                if (mx != x) {
                    newx = ((mx - x) > 0) ? mx - 0.5 : mx + 0.5;
                }

                if (my != y) {
                    newy = ((my - y) > 0) ? my - 0.5 : my + 0.5;
                }

                if (mz != z) {
                    newz = ((mz - z) > 0) ? mz - 0.5 : mz + 0.5;
                }
    ...
    public void attackLivingEntity(LivingEntity ent) {
        try {
            if (((ent instanceof Player) && !humans) || ((ent instanceof Monster) && !mobs)) {
                return;
            }

            if ((ent.getHealth() - damage) < 0) {
                ent.setHealth(0);
                following = null;
                attacking = null;
