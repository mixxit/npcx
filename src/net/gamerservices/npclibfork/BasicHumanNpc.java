...
public class BasicHumanNpc extends BasicNpc {
        ...
        if (this.hp > 0) {
            ...
            if (this.parent.pathgroup != null) {
                ...
                if (getFollow() == null && getAggro() == null) {
                    // System.out.println("npcx : moving  ["+ spawnx + "] ["+
                    // spawny + "] ["+ spawnz + "]");
                    // Requip my armour
    ...
    private boolean isBlockWall(Block block) {
        ...
        if (babove.getType() == Material.AIR) { return false; }
        // assume the worse
    ...
    private boolean isBlockFloatingAir(Block block) {
        ...
        if (block.getType() == Material.AIR && blockbelow.getType() != Material.AIR) return false;
        // assume worse
        return true;
    }
    ...
    private Location findPlatformDown(Location belowloc) {
        ...
        while (current < max) {
            ...
            if (this.getBukkitEntity().getWorld().getBlockAt(newloc).getType() != Material.AIR) { return newloc;
            ...
            if (ent.getHealth() > 0) {
                if ((ent.getHealth() - dmg) <= 0) {
                    ...
                    if (ent instanceof Player) {
                        // ((Player) ent).getServer().broadcastMessage(((Player)
                        // ent).getName() + " was slaughtered by " + getName() +
                        // ".");
