                ...
                for (myTriggerword tw : triggerwords.values()) {
                    ...
                    if (word.toLowerCase().contains(tw.word.toLowerCase())) {
                        ...
                        if (tw.response.toLowerCase().contains(npcattack.toLowerCase())) {
                            ...
                            npc.attacking = myplayer.player;
                            npc.following = myplayer.player;
                            return;
                        }
                                    ...
                                    if (cost > 0) {
                                        if (amount != 0) {
                                            ...
                                            if (item.item.getAmount() <= amount) {
                                                ...
                                                i.item = new ItemStack(item.item.getType());
                                                i.item.setAmount(amount);
                                                i.item.setTypeId(item.item.getTypeId());
                                                amount = 0;
                                                basket.add(i);
