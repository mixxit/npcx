...
public class myNPC {
    ...
    public void parseChat(myPlayer myplayer, String message) {
        ...
        int count = 0;
        int size = 0;
        // myplayer.player.sendMessage("Parsing:" + message + ":" +
        // Integer.toString(this.triggerwords.size()));
        ...
        for (String word : message2.split(" ")) {
            ...
            if (count == 0) {
                if (triggerwords != null) {
                    for (myTriggerword tw : triggerwords.values()) {
                        // myplayer.player.sendMessage("Test:" + word + ":"+
                        // tw.word);
    ...
    private void parseChatGlobalCommands(myPlayer myplayer, String message) {
        ...
        if (arg1.matches("give")) {
            ...
            // check if the item type is null incase it didnt find the material
            // type
            ...
            for (ItemStack curitem : myplayer.player.getInventory().getContents()) {
                ...
                try {
                    ...
                    if (curitem.getTypeId() == item.getTypeId()) {
                        ...
                        // player.player.sendMessage(npc.getName() +
                        // " says to you, '"+ curitem.getTypeId()
                        // +"/"+curitem.getAmount() +"'");
                    }
                } catch (NullPointerException e) {
                    // empty slot
            ...
            if (merchant.merchantentries != null) {
                ...
                for (myMerchant_entry entry : merchant.merchantentries) {
                    if (entry.itemid == itemid) {
                        ...
                        // System.out.println("About to sell: " +
                        // i.getType().name() + ":"+ entry.amount);
                        return i;
    ...
    private int getMerchantPriceBuyAt(int typeId) {
        ...
        if (merchant != null) {
            if (merchant.merchantentries != null) {
                for (myMerchant_entry entry : merchant.merchantentries) {
                    if (entry.itemid == typeId) { return entry.pricebuy; }
                }
            }
        }
        return amount;
    ...
    private int getMerchantPriceSellAt(int typeId) {
        ...
        if (merchant != null) {
            if (merchant.merchantentries != null) {
                for (myMerchant_entry entry : merchant.merchantentries) {
                    if (entry.itemid == typeId) { return entry.pricesell; }
                }
            }
        }
        return amount;
    ...
    public BasicHumanNpc Spawn(String id2, String name2, World world, double x, double y, double z, Double yaw, Double pitch) {
        // TODO Auto-generated method stub
            ...
            if (player.player == p) {
                ...
                if (player.target != null) {
                    ...
                    if (this.parent != null) {
                        ...
                        if (this.spawngroup != null) tGPID = this.spawngroup.id;
                        if (this.faction != null) tFID = this.faction.id;
                        if (this.pathgroup != null) tPGID = this.pathgroup.id;
                        if (this.loottable != null) tLTID = this.loottable.id;
                        if (this.merchant != null) tMID = this.merchant.id;
